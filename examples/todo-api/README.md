...
Registry - это интерфейс для хранения мемоизированных результатов моделей. Обычно используется `Map<Key, Promise<unknown>>()`, который реализует этот интерфейс. **Важно**: Registry должен создаваться **для каждого HTTP-запроса отдельно**. Никогда не переиспользуйте registry между разными запросами - это приведёт к утечке данных и неправильной мемоизации.
